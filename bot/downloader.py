...
async def _send_file(sender, file_path, caption="", progress=None):
    ...
    if os.path.getsize(file_path) > _SIZE_LIMIT:
        holder = await app.send_message(sender, "Böyük fayl yüklənir...")
...
async def _download_tme(message, sender, chat_id, message_id, is_story):
    ...
    if not userbot:
        return await message.reply(
            f"{Msg.EMOJI_WARNING} Bu link yalnız userbotunuzun çata biləcəyi bir çata aiddir, "
            f"lakin userbot qoşulu deyil. Onu yenidən aktiv etmək üçün /restart istifadə edin."
        )
    try:
        src = (
        ...
        if not src.media:
            return await app.send_message(sender, src.text or "(məzmun yoxdur)")
        msg = await app.send_message(sender, f"{Msg.EMOJI_DOWNLOAD} Yüklənir...")
        ...
        await message.reply(f"{Msg.EMOJI_ERROR} Mesaj alınarkən xəta: {e}")
...
async def _download_http(message, sender, url):
    ...
    try:
        ...
        await message.reply(f"{Msg.EMOJI_ERROR} Fayl alınarkən xəta: {e}")
    finally:
...
async def _start_batch_download(message, sender, chat_identifier):
    ...
    if not userbot:
        return await message.reply(
            f"{Msg.EMOJI_WARNING} Userbotunuz qoşulu deyil, ona görə bu çatı oxuya bilmirəm. "
            f"Onu yenidən aktiv etmək üçün /restart istifadə edin."
        )
    try:
        ...
        return await message.reply(
            f"{Msg.EMOJI_ERROR} Çata giriş mümkün deyil: {e}\n\n"
            "<b>Bunlardan birini daxil edin:</b>\n"
            "• @username\n• çat id (məsələn, -1001234567890)\n• istifadəçi id\n\n"
            "<b>Qeyd:</b> siz çatın üzvü olmalısınız.",
            parse_mode=ParseMode.HTML,
        )

    _batch_downloads[sender] = chat.id
    buttons = InlineKeyboardMarkup([
        [InlineKeyboardButton("📥 Son 10", callback_data="batch_10"),
         InlineKeyboardButton("📥 Son 25", callback_data="batch_25")],
        [InlineKeyboardButton("📥 Son 50", callback_data="batch_50"),
         InlineKeyboardButton("📥 Son 100", callback_data="batch_100")],
        [InlineKeyboardButton("📝 Xüsusi Aralıq", callback_data="batch_custom")],
        [InlineKeyboardButton("❌ Ləğv et", callback_data="batch_cancel")],
    ])
    title = getattr(chat, 'title', None) or getattr(chat, 'first_name', 'Çat')
    await message.reply(
        f"{Msg.EMOJI_FOLDER} <b>Çat:</b> {title}\n"
        f"🆔 <b>ID:</b> <code>{chat.id}</code>\n\n"
        "Neçə mesaj yüklənəcəyini seçin:",
...
async def batch_download_callback(client, callback_query: CallbackQuery):
    ...
    if action == 'cancel':
        ...
        return await callback_query.edit_message_text(f"{Msg.EMOJI_ERROR} Toplu yükləmə ləğv edildi.")

    if sender not in _batch_downloads:
        return await callback_query.answer(f"{Msg.EMOJI_WARNING} Sessiya bitib. Zəhmət olmasa, yenidən cəhd edin.", show_alert=True)

    chat_id = _batch_downloads[sender]
    userbot = clients.get(sender)
    if not userbot:
        _batch_downloads.pop(sender, None)
        return await callback_query.edit_message_text(f"{Msg.EMOJI_WARNING} Userbot qoşulu deyil. /restart istifadə edin.")

    try:
        if action == 'custom':
            await callback_query.edit_message_text(
                f"{Msg.EMOJI_NOTE} <b>Xüsusi Aralıq Rejimi</b>\n\n"
                "Mesaj ID-lərini belə göndərin:\n"
                "• Tək: <code>12345</code>\n"
                "• Aralıq: <code>100:150</code>\n"
                "• Çoxlu: <code>100,105,110</code>\n\n"
                "Cavab vermək üçün 60 saniyəniz var.",
                parse_mode=ParseMode.HTML,
            )
            ...
            if response is None:
                return await app.send_message(sender, f"{Msg.EMOJI_LOADING} Vaxt bitdi. Zəhmət olmasa, yenidən cəhd edin.")
            raw = (response.text or "").strip()
            if raw.startswith('/'):
                return await app.send_message(sender, f"{Msg.EMOJI_ERROR} Ləğv edildi.")
            try:
            ...
            count = int(action)
            await callback_query.edit_message_text(f"{Msg.EMOJI_DOWNLOAD} Son {count} mesaj yüklənir...")
            await _download_messages_batch(userbot, sender, chat_id, count=count)
    except Exception as e:
        await callback_query.edit_message_text(f"{Msg.EMOJI_ERROR} Xəta: {e}")
    finally:
        _batch_downloads.pop(sender, None)
...
def _parse_message_ids(raw):
    ...
    if ':' in raw:
        try:
            ...
            raise ValueError("Yanlış aralıq formatı. start:end istifadə edin")
        if start > end:
            start, end = end, start
    ...
    if ',' in raw:
        try:
            ...
            raise ValueError("Yanlış format. Vergüllə ayrılmış rəqəmlər istifadə edin.")
    try:
        return [int(raw)]
    except Exception:
        raise ValueError("Yanlış mesaj ID.")
...
async def _download_messages_batch(userbot, sender, chat_id, message_ids=None, count=None):
    ...
    user_dir = _user_dir(sender)
    status = await app.send_message(sender, f"{Msg.EMOJI_LOADING} Toplu yükləmə başlayır...")
    ...
    if message_ids:
        for i in range(0, len(message_ids), 100):
            ...
            try:
                ...
                logger.error(f"[TOPLU] hissə əldə etmə xətası: {e}")
    ...
    if not messages:
        return await status.edit_text(f"{Msg.EMOJI_ERROR} Heç bir mesaj tapılmadı.")

    await status.edit_text(f"{Msg.EMOJI_FOLDER} {len(messages)} mesaj tapıldı. Emal edilir...")
        ...
        try:
            if idx % 10 == 0:
                await status.edit_text(
                    f"{Msg.EMOJI_DOWNLOAD} Gedişat: {idx}/{len(messages)}\n"
                    f"{Msg.EMOJI_SUCCESS} Uğurlu: {success} | {Msg.EMOJI_ERROR} Uğursuz: {failed}"
                )
            if m.media:
            ...
            await status.edit_text(f"{Msg.EMOJI_LOADING} Flood gözləməsi: {e.value}s. Fasilə verilir...")
            await asyncio.sleep(e.value)
        except Exception as e:
            logger.error(f"[TOPLU] {idx} nömrəli mesajda xəta: {e}")
            failed += 1
            await asyncio.sleep(1)

    await status.edit_text(
        f"{Msg.EMOJI_SUCCESS} <b>Toplu Yükləmə Tamamlandı!</b>\n\n"
        f"• Cəmi: {len(messages)}\n• Uğurlu: {success}\n• Uğursuz: {failed}",
        parse_mode=ParseMode.HTML,
    )
