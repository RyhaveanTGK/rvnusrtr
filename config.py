...
def _init_storage():
    ...
    if backend == 'mongo':
        try:
            ...
            logger.info("MongoDB qoşuldu (baza: %s)", DB_NAME)
            return client, client[DB_NAME]["user_sessions"]
        except Exception as e:
            logger.warning(
                "MongoDB bağlantısı alınmadı (%s); müvəqqəti yaddaşa keçilir. "
                "Məlumatlar restart-dan sonra itəcək.", e
            )
            return None, MemoryCollection()
    if backend == 'sqlite':
        logger.info("SQLite yaddaşı istifadə olunur: %s", SQLITE_PATH)
        return None, SqliteCollection(SQLITE_PATH)
    logger.info("Müvəqqəti (in-memory) yaddaş istifadə olunur.")
    return None, MemoryCollection()
...
def get_collection(name):
    """MongoDB kolleksiyası qaytarır; Mongo yoxdursa müvəqqəti yaddaş verir."""
    if db is not None:
        return db[name]
    return MemoryCollection()


# Lisenziyalar (icazə verilmiş istifadəçilər) və userbot sessiyaları
licenses = get_collection("licenses")
userbot_sessions = get_collection("userbot_sessions")
...
# Bot sahibləri (lisenziya verə bilən adminlər) — vergüllə ayrılmış ID siyahısı
OWNER_ID = int(os.getenv('OWNER_ID', '0') or 0)
BOT_ADMINS = [
    int(x) for x in os.getenv('BOT_ADMINS', '').replace(' ', '').split(',') if x.strip().isdigit()
]
if OWNER_ID and OWNER_ID not in BOT_ADMINS:
    BOT_ADMINS.append(OWNER_ID)

# Lisenziyanın standart müddəti (gün)
...
# Qurulumda avtomatik qoşulacaq kanal/qruplar
AUTO_JOIN_CHATS = [
    c.strip() for c in os.getenv(
        'AUTO_JOIN_CHATS',
        'https://t.me/ryhaveanupdates,https://t.me/RyhaveanTeam'
    ).split(',') if c.strip()
]

# Rəsmi Ryhavean linkləri
UPDATES_CHANNEL_URL = os.getenv('UPDATES_CHANNEL_URL', 'https://t.me/ryhaveanupdates')
COMMUNITY_GROUP_URL = os.getenv('COMMUNITY_GROUP_URL', 'https://t.me/RyhaveanTeam')
