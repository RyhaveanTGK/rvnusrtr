...
async def afk_handler(client, message):
    ...
    await message.reply(
        f"<b>AFK-yam {afk_time}\nSəbəb:</b> <i>{afk_info['reason']}</i>"
    )
...
async def afk(client, message):
    ...
    await message.edit(f"<b>AFK-a keçirəm.\n" f"Səbəb:</b> <i>{reason}</i>")
...
async def unafk(client, message):
    ...
    if afk_info and is_afk:
        ...
        await message.edit(
            f"<b>Artıq AFK deyiləm.\n" f"AFK olduğum müddət: {afk_time}</b>"
        )
        afk_info = {
            "start": 0,
            "is_afk": False,
        ...
        await message.edit("<b>Siz AFK deyildiniz</b>")
