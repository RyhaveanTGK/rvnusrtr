...
async def ask_handler(client: Client, message: Message):
    ...
    if not ai_backend.is_configured():
        await edit_or_reply(
            message,
            styled_error("`.ask` istifadə etmək üçün `.env` faylınızda həm `AI_API_KEY`, həm də `AI_BASE_URL` təyin edilməlidir."),
        )
        return
    ...
    if not query:
        await edit_or_reply(
            message,
            styled_error("Bir sual verin və ya mesaja cavab verin.\n\n**İstifadə:** `.ask <sual>`"),
        )
        return

    status_msg = await edit_or_reply(
        message, _format_answer(message, "🧠 **AI düşünür...**")
    )
    ...
    try:
        ...
        await _safe_edit(
            status_msg,
            _format_answer(message, styled_error(f"Sorğu {ASK_TIMEOUT} saniyədən sonra vaxtı bitdi.")),
        )
        return
    except ai_backend.AgentError as e:
        await _safe_edit(
            status_msg,
            _format_answer(message, styled_error(f"AI xidmət xətası: {ai_backend.scrub(str(e))}")),
        )
        return
...
async def ask_clear_handler(client: Client, message: Message):
    ...
    if ai_backend.clear_chat_history(message.chat.id):
        await edit_or_reply(message, "🧹 **Söhbət yaddaşı təmizləndi.** Yenidən başlanılır.")
    else:
        await edit_or_reply(message, "🧹 **Söhbət yaddaşı artıq boşdur.**")
...
async def ask_model_handler(client: Client, message: Message):
    ...
    status = await edit_or_reply(message, "🔄 **Model məlumatı əldə edilir...**")
    info = await asyncio.to_thread(ai_backend.get_active_model_info, force_refresh)

    if info.get("is_cheapest"):
        mode = "Avtomatik (ən ucuz) 🏆"
    elif info.get("error"):
        mode = f"Konfiqurasiya edilmiş standart ⚙️ (qiymət axtarışı uğursuz oldu: {info['error']})"
    else:
        mode = "Konfiqurasiya edilmiş standart ⚙️"

    if info.get("prompt_price_1m") or info.get("completion_price_1m"):
        price = (
            f"${info['prompt_price_1m']:.4f} giriş / "
            f"${info['completion_price_1m']:.4f} çıxış 1M token üçün"
        )
    else:
        price = "Yoxdur"

    shell_state = "aktivdir ⚠️" if AGENT_ALLOW_SHELL else "deaktivdir 🔒"
    moderation_state = "aktivdir ⚠️" if AGENT_ALLOW_MODERATION else "deaktivdir 🔒"
    api_state = "aktivdir ⚠️" if AGENT_ALLOW_TELEGRAM_API else "deaktivdir 🔒"
    ...
    await _safe_edit(
        ...
        f"• **Seçim:** {mode}\n"
        f"• **Qiymət:** {price}\n"
        f"• **Shell aləti:** {shell_state}\n"
        f"• **Moderasiya alətləri:** {moderation_state}\n"
        f"• **Tam Telegram API:** {api_state}\n\n"
        "💡 Qiyməti yenidən sorğulamaq üçün `.askmodel refresh` istifadə edin.",
    )
