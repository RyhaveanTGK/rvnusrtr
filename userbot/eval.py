...
RUNNING = "<b>Eval İfadəsi:</b>\n<pre>{}</pre>\n<b>İcra edilir...</b>"
ERROR = "<b>Eval İfadəsi:</b>\n<pre>{}</pre>\n<b>Xəta:</b>\n<pre>{}</pre>"
SUCCESS = "<b>Eval İfadəsi:</b>\n<pre>{}</pre>\n<b>Uğurlu</b>"
RESULT = "<b>Eval İfadəsi:</b>\n<pre>{}</pre>\n<b>Nəticə:</b>\n<pre>{}</pre>"
...
async def eval_expression(client, message):
    ...
    if not message.text or len(message.command) < 2:
        await message.reply("Zəhmət olmasa icra ediləcək kod daxil edin.")
        return
...
"""
        ...
        if not output:
            try:
                ...
                await handle_long_message(client, response_msg, code, "Uğurlu, nəticə yoxdur")
            return
...
async def handle_long_message(client, original_msg, code, output, is_error=False):
    ...
    try:
        ...
        with open(file_name, "w", encoding="utf-8") as file:
            file.write(f"Eval İfadəsi:\n{code}\n\n")
            file.write(f"{'Xəta' if is_error else 'Nəticə'}:\n{output}")
        
        # Edit message to indicate file upload
        await client.edit_message_text(
            chat_id=original_msg.chat.id,
            message_id=original_msg.id,
            text=f"<b>Eval İfadəsi:</b>\n<pre>{html.escape(code[:500])}{'...' if len(code) > 500 else ''}</pre>\n<b>{'Xəta' if is_error else 'Nəticə'} çox böyükdür, fayl kimi yüklənir...</b>"
        )
        ...
        await client.send_document(
            chat_id=original_msg.chat.id,
            document=file_name,
            caption=f"{'Xəta' if is_error else 'Nəticə'} nəticəsi eval əmri üçün"
        )
        ...
        await client.edit_message_text(
            chat_id=original_msg.chat.id,
            message_id=original_msg.id,
            text=f"<b>Eval İfadəsi:</b>\n<pre>{html.escape(code[:500])}{'...' if len(code) > 500 else ''}</pre>\n<b>Nəticə göstərmək üçün çox böyükdür və fayl yükləmə uğursuz oldu:</b> {str(file_error)}"
        )
