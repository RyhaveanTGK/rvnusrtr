...
async def fontss(client, message):
    if message.text.startswith("/fonts"):
        return await message.edit_text("Mövcud şrift stillərinə [buradan](https://telegra.ph/AVAILABLE-FONTS-STYLES-05-16-3) baxın\n\nMisal: _frozen salam")
    ...
    try:
      if match:
        ...
        await bot.send_message(sender, f"XƏTA: {e}")
