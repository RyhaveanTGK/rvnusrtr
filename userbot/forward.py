...
async def forward_message_handler(client, message):
        ...
        channel_details = f"<b>{channel_name}</b> {channel_username} <a href='{message_link}'>Mesaja keçid</a>"
        try:
          ...
          # Display current size and total size
                           progress_message += f"Ölçü: {current / (1024 * 1024):.2f} MB / {total / (1024 * 1024):.2f} MB"
          ...
          # Combine progress bar and message
             ...
             # Create a message with HTML formatting for better appearance
                        ...
                        msg = await bot.send_message(sender, "Media/sənəd yüklənir......")
                        type_of="Yüklənir"
