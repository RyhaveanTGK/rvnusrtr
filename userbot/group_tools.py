...
async def promote_user(client, message):
    ...
    if len(command_parts) >= 2:
        ...
        elif promotion_type in ("basic", "nub"):
            permissions = {
        ...
        try:
            ...
            if promotion_type == "less":
              return await message.edit("İstifadəçinin admin statusu uğurla ləğv edildi.")
            await message.edit("İstifadəçi uğurla admin edildi.")
            await asyncio.sleep(2)
            await client.set_administrator_title(chat_id, user_id, title)
        except Exception as e:
            await message.edit(f"İstifadəçini admin etmək uğursuz oldu: {e}")
    else:
        await message.edit("Yanlış əmr istifadəsi. Zəhmət olmasa admin növü və titul daxil edin.")
...
async def inv(client, message):
    sender = client.me.id
    Man = await message.edit_text("`Emal edilir . . .`")
    text = message.text.split(" ", 1)
    queryy = text[1]
    chat = await client.get_chat(queryy)
    tgchat = message.chat
    await Man.edit_text(f"{chat.username} adlı söhbətdən istifadəçilər dəvət edilir")
    async for member in client.get_chat_members(chat.id):
        ...
        if user.status in zxb:
            try:
              ...
              mg = await bot.send_message(sender, f"**XƏTA:** `{e}`")
              break
...
async def invite_to_voice_chat(client, message):
    chat_id = message.chat.id
    users = []
    await message.edit("İstifadəçilər səsli söhbətə dəvət edilməyə başlanır...")
    ...
    try:
        ...
        if not call:
            await message.edit("Aktiv qrup zəngi tapılmadı.")
            return
    except Exception as e:
        await message.edit(f"Qrup zəngini əldə etmə xətası: {str(e)}")
        return
    ...
    await message.edit(f"İstifadəçilərin dəvəti tamamlandı. Ümumi dəvət edilən: {z}")
...
async def adminlist(client, message):
    ...
    teks = f"**{grup.title} qrupunun adminləri**\n"
    teks += "╒═══「 Yaradıcı 」\n"
    ...
    teks += f"<blockquote>╞══「 {len(admin)} İnsan Admin 」\n"
    await add_user_info(admin)
    teks += f"\n\n<blockquote>╞══「 {len(badmin)} Bot Admin 」\n"
    await add_user_info(badmin)

    teks += f"\n\n<blockquote>╘══「 Toplam {totaladmins} Admin 」</blockquote>"
...
async def id_command(client, message):
    lines = [f"💬 **Söhbət ID:** `{message.chat.id}`"]
    reply = message.reply_to_message
    if reply and reply.from_user:
        lines.append(f"👤 **Cavab verilən istifadəçi:** `{reply.from_user.id}`")
    elif message.from_user:
        lines.append(f"👤 **Sizin ID:** `{message.from_user.id}`")
    if reply:
        lines.append(f"#️⃣ **Mesaj ID:** `{reply.id}`")
        # media file IDs
        _MEDIA = ("photo", "video", "audio", "voice", "video_note", "animation", "document", "sticker")
        for attr in _MEDIA:
            obj = getattr(reply, attr, None)
            if obj:
                lines.append(f"📎 **Fayl ID ({attr}):** `{obj.file_id}`")
                thumb = getattr(obj, "thumbs", None) or getattr(obj, "thumb", None)
                if thumb:
                    t = thumb[0] if isinstance(thumb, list) else thumb
                    lines.append(f"🖼 **Miniatür Fayl ID:** `{t.file_id}`")
                break
    await message.edit("\n".join(lines))
...
async def leave_command(client, message):
    chat_id = message.chat.id
    await message.edit("👋 Bu söhbətdən çıxılır...")
    try:
        await client.leave_chat(chat_id)
    except Exception as e:
        await client.send_message(client.me.id, styled_error(f"`{chat_id}` söhbətindən çıxmaq uğursuz oldu: {e}"))
