...
async def help_handler(client, message):
    ...
    try:
        ...
        await edit_or_reply(
            message,
            f"Naməlum Əmr\n\n"
            f"┃ {f'{cmd_name} üçün kömək tapılmadı'}\n"
            f"┃ 💡 {f'Bütün kateqoriyaları görmək üçün {prefix}help istifadə edin'}\n"
            f"╰━━━━━━━━━━━━━━━━━━━━╯"
        )

    except Exception as e:
        logger.error(f"[HELP] Error: {e}")
        await edit_or_reply(message, styled_error(f"Kömək xətası: {str(e)[:50]}"))
