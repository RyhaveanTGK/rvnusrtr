...
async def inline_handler_ban(client, message):
    if apps.get("app") is None:
        await message.edit_text("❌ Yardımçı bot konfiqurasiya edilməyib/başladılmayıb. İnline əmr icra edilə bilmir.")
        return
    try:
        ...
        await message.reply(styled_error(f"Xəta: {e}"))
...
async def unban_all_users(client, message):
    ...
    try:
        ...
        try:
            ...
            await client.send_message(chat_id, styled_error(f"İcazə yoxlaması uğursuz oldu: {str(e)}"))
            return
        ...
        status_msg = await client.send_message(
            chat_id,
            f"🔄 {f'{chat.title} üçün ban ləğvi başlayır...'}"
        )
        ...
        try:
            await status_msg.edit(f"🔄 İstifadəçilərin banı ləğv edilir...")
            ...
            async for member in client.get_chat_members(chat_id, filter=enums.ChatMembersFilter.BANNED):
                ...
                try:
                    ...
                    if total_processed % 10 == 0:
                        progress_message = f"""🔄 Ban ləğvi davam edir...

📊 Emal edildi: {total_processed}
✅ Ban ləğv olundu: {unbanned_count}
❌ Uğursuz: {failed_count}
📈 Uğur Nisbəti: {(unbanned_count/total_processed)*100:.1f}%"""
            ...
            # Final result
            final_message = f"""✅ Bütün Banların Ləğvi Tamamlandı

📊 **Nəticələr:**
👥 Ümumi Emal Edilən: {total_processed}
✅ Uğurla Ban Ləğv Edilən: {unbanned_count}
❌ Ban Ləğvi Uğursuz Olan: {failed_count}
📈 Uğur Nisbəti: {(unbanned_count/total_processed)*100:.1f}%

🎉 {chat.title} söhbətindəki bütün uyğun istifadəçilərin banı ləğv edildi"""

            await status_msg.edit(final_message)

        except Exception as e:
            await status_msg.edit(styled_error(f"Ban ləğvi xətası: {str(e)}"))

    except Exception as e:
        try:
            await client.send_message(
                message.chat.id,
                styled_error(f"Bütün banların ləğvi uğursuz oldu: {str(e)}")
            )
