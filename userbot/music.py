...
async def progress_bar(current, total, client, msg, type_of, filename, start_time):
    ...
    try:
            ...
            # Format message
            progress_message += (
                f"Sürət: {speed:.2f} MB/s\n"
                f"Qalan vaxt: {time_left:.2f}s\n"
                f"Ölçü: {current/1024/1024:.2f}MB / {total/1024/1024:.2f}MB\n"
                f"[{progress_bar_text}]"
            )
...
async def play_handler_func(client, message):
    ...
    if message.chat.type not in [ChatType.GROUP, ChatType.SUPERGROUP]:
        await message.edit(
            f"Yalnız Qrup\n\n"
            f"┃ Musiqi qrup və superqruplarda işləyir\n"
            f"╰▸ Musiqi yayımı üçün qrupa qoşulun"
        )
        return
    ...
    massage = await message.reply(
        f"╭━━ {Msg.EMOJI_SEARCH} AXTARIŞ ━━╮\n"
        f"┃ Mahnınız axtarılır...\n"
        f"╰━━━━━━━━━━━━━━━━━╯"
    )
    ...
    if message.reply_to_message and message.reply_to_message.media:
        ...
        if media_msg.video:
            ...
            media_type = "video"
            title = media.file_name or "Telegram Videosu"
        ...
        # Audio handling
            ...
            media_type = "audio"
            title = media.title or "Telegram Audiosu"
            duration = media.duration
        ...
        # Video note handling
            ...
            for attr in doc.attributes:
                if isinstance(attr, DocumentAttributeVideo):
                    media_type = "video"
                    title = doc.file_name or "Telegram Videosu"
                    duration = attr.duration
                elif isinstance(attr, DocumentAttributeAudio):
                    media_type = "audio"
                    title = doc.file_name or "Telegram Audiosu"
                    duration = attr.duration
        else:
            await massage.edit(f"{Msg.EMOJI_ERROR} Dəstəklənməyən media növü")
        ...
        if not youtube_link:
            try:
                await massage.edit(
                f"{Msg.EMOJI_SEARCH} Nəticə Tapılmadı\n\n"
                f"┃ Uyğun mahnı tapılmadı\n"
                f"╰▸ Başqa axtarış sorğusu sınayın"
            )
                return await remove_active_chat(client, target_chat_id)
            except Exception as e:
        ...
        try:
            await massage.edit(
                f"{Msg.EMOJI_QUESTION} Sorğu Yoxdur\n\n"
                f"┃ Zəhmət olmasa axtarış sorğusu daxil edin\n"
                f"╰▸ İstifadə: `[prefix]play <mahnı adı>`"
            )
            return await remove_active_chat(client, target_chat_id)
        except Exception as e:
...
async def end_handler_func(client, message):
    ...
    try:
        ...
        if is_active:
            ...
            await client.send_message(message.chat.id, Msg.card("Yayım Bitdi", ["Növbə təmizləndi və zəngdən çıxıldı.", f"Kim: {message.from_user.mention()}"], emoji=Msg.EMOJI_SUCCESS), reply_to_message_id=message.id)
            if call_py:
                await call_py.leave_call(message.chat.id)
            if message.chat.id in playing:
                playing[message.chat.id].clear()
        else:
            await client.send_message(message.chat.id, Msg.card("Yayım Yoxdur", ["Hazırda heç nə oxudulmur."], emoji=Msg.EMOJI_WARNING, footer="Başlamaq üçün /play istifadə edin"), reply_to_message_id=message.id)
            await remove_active_chat(client, message.chat.id)
            if call_py:
                await call_py.leave_call(message.chat.id)
            if message.chat.id in playing:
    ...
    try:
        ...
        if song_queue and message.chat.id in song_queue:
            if len(song_queue[message.chat.id]) > 0:
                ...
                await client.send_message(message.chat.id, Msg.card("Keçilir", ["Növbəti mahnı yüklənir...", f"Kim: {message.from_user.mention()}"], emoji=Msg.EMOJI_INFO), reply_to_message_id=message.id)
                ...
                if call_py:
                    await call_py.leave_call(message.chat.id)
                await remove_active_chat(client, message.chat.id)
                await client.send_message(message.chat.id, Msg.card("Keçildi", ["Növbə artıq boşdur.", f"Kim: {message.from_user.mention()}"], emoji=Msg.EMOJI_SUCCESS), reply_to_message_id=message.id)
                if message.chat.id in playing:
                    playing[message.chat.id].clear()
        else:
            if call_py:
...
async def pause_handler_func(client, message):
    try:
        ...
        if await is_active_chat(client, message.chat.id):
            if call_py:
                await call_py.pause(message.chat.id)
            await client.send_message(message.chat.id, Msg.card("Dayandırıldı", ["Davam etmək üçün /resume istifadə edin.", f"Kim: {message.from_user.mention()}"], emoji=Msg.EMOJI_INFO), reply_to_message_id=message.id)
        else:
            await client.send_message(message.chat.id, Msg.card("Yayım Yoxdur", ["Hazırda heç nə oxudulmur."], emoji=Msg.EMOJI_WARNING, footer="Başlamaq üçün /play istifadə edin"), reply_to_message_id=message.id)
    except NotInCallError:
        await client.send_message(message.chat.id, Msg.card("Yayım Yoxdur", ["Hazırda heç nə oxudulmur."], emoji=Msg.EMOJI_WARNING, footer="Başlamaq üçün /play istifadə edin"), reply_to_message_id=message.id)
...
async def resume_handler_func(client, message):
    try:
        ...
        if await is_active_chat(client, message.chat.id):
            ...
            await client.send_message(message.chat.id, Msg.card("Davam Etdirildi", ["Dayandırmaq üçün /pause istifadə edin.", f"Kim: {message.from_user.mention()}"], emoji=Msg.EMOJI_SUCCESS), reply_to_message_id=message.id)
        else:
            await client.send_message(message.chat.id, Msg.card("Yayım Yoxdur", ["Hazırda heç nə oxudulmur."], emoji=Msg.EMOJI_WARNING, footer="Başlamaq üçün /play istifadə edin"), reply_to_message_id=message.id)
    except NotInCallError:
        await client.send_message(message.chat.id, Msg.card("Yayım Yoxdur", ["Hazırda heç nə oxudulmur."], emoji=Msg.EMOJI_WARNING, footer="Başlamaq üçün /play istifadə edin"), reply_to_message_id=message.id)
    ...
    try:
        ...
        if len(command_parts) != 2:
            await client.send_message(
                message.chat.id,
                Msg.card("Yanlış Təkrar Əmri", ["Zəhmət olmasa təkrar sayını qeyd edin."], emoji=Msg.EMOJI_ERROR, footer="İstifadə: /loop <say>"), reply_to_message_id=message.id
            )
            return
        ...
        try:
            ...
            if loop_count <= 0 or loop_count > 20:
                await client.send_message(
                    message.chat.id,
                    Msg.card("Yanlış Təkrar Sayı", ["Təkrar sayı 1 ilə 20 arasında olmalıdır."], emoji=Msg.EMOJI_ERROR), reply_to_message_id=message.id
                )
                return
        except ValueError:
            await client.send_message(
                message.chat.id,
                Msg.card("Yanlış Təkrar Sayı", ["Düzgün bir rəqəm daxil edin."], emoji=Msg.EMOJI_ERROR), reply_to_message_id=message.id
            )
            return
        ...
        if message.chat.id in playing and playing[message.chat.id]:
            ...
            await client.send_message(
                message.chat.id,
                Msg.card("Təkrar Aktivdir", [f"Cari mahnı {loop_count} dəfə təkrarlanacaq.", f"Kim: {message.from_user.mention()}"], emoji=Msg.EMOJI_SUCCESS), reply_to_message_id=message.id
            )
        else:
            await client.send_message(
                message.chat.id,
                Msg.card("Yayım Yoxdur", ["Köməkçi hazırda heç nə yayımlamır."], emoji=Msg.EMOJI_WARNING, footer="Başlamaq üçün /play istifadə edin"), reply_to_message_id=message.id
            )
        ...
        await client.send_message(
            message.chat.id,
            Msg.card("Musiqi Xətası", ["Nəsə səhv getdi."], emoji=Msg.EMOJI_ERROR), reply_to_message_id=message.id
        )
...
async def queue_command(client, message):
    ...
    try:
        ...
        if not song_queue or not song_queue.get(message.chat.id):
            await message.edit(Msg.card("Növbə Boşdur", ["Növbədə mahnı yoxdur."], emoji=Msg.EMOJI_INFO, footer="Əlavə etmək üçün [prefix]play <mahnı>"))
            return
        ...
        queue_text = f"╭━━ {Msg.EMOJI_MUSIC} MUSİQİ NÖVBƏSİ ({len(current_queue)}) ━━╮\n\n"
        ...
        if len(current_queue) > 10:
            queue_text += f"\n... və daha {len(current_queue) - 10} mahnı"
        
        await message.edit(queue_text)
    except Exception as e:
        await message.edit(f"❌ **Xəta:** {str(e)}")

# Music.py Commands and Categories
music_commands = {
    'play': '**Audio Oxut** - YouTube və ya digər mənbələrdən audio oxudur.\n\n**İstifadə:** `[prefix]play <sorğu/url>` və ya mediaya cavab verin\n**Nümunə:** `[prefix]play Never Gonna Give You Up`',
    'vplay': '**Video Oxut** - YouTube-dan səsli video oxudur.\n\n**İstifadə:** `[prefix]vplay <sorğu/url>` və ya mediaya cavab verin\n**Nümunə:** `[prefix]vplay musiqi videosu`',
    'playforce': '**Məcburi Audio Oxut** - Cari növbəni ötürərək audio oxudur.\n\n**İstifadə:** `[prefix]playforce <sorğu/url>`\n**Nümunə:** `[prefix]playforce mahnı adı`',
    'vplayforce': '**Məcburi Video Oxut** - Cari növbəni ötürərək video oxudur.\n\n**İstifadə:** `[prefix]vplayforce <sorğu/url>`\n**Nümunə:** `[prefix]vplayforce video adı`',
    'pause': '**Yayımı Dayandır** - Cari musiqi/videonu dayandırır.\n\n**İstifadə:** `[prefix]pause`\n**Qeyd:** Davam etmək üçün `[prefix]resume` istifadə edin',
    'resume': '**Yayımı Davam Etdir** - Dayandırılmış musiqi/videonu davam etdirir.\n\n**İstifadə:** `[prefix]resume`\n**Qeyd:** Dayandırmaq üçün `[prefix]pause` istifadə edin',
    'skip': '**Mahnını Keç** - Növbədəki növbəti mahnıya keçir.\n\n**İstifadə:** `[prefix]skip`\n**Qeyd:** Cari mahnını keçib növbətini oxudur',
    'end': '**Yayımı Dayandır** - Musiqini dayandırır və növbəni təmizləyir.\n\n**İstifadə:** `[prefix]end`\n**Qeyd:** Bütün yayımı dayandırır və növbəni təmizləyir',
    'loop': '**Mahnını Təkrarla** - Cari mahnını bir neçə dəfə təkrarlayır.\n\n**İstifadə:** `[prefix]loop <say>`\n**Nümunə:** `[prefix]loop 5` (5 dəfə təkrarlayır)\n**Qeyd:** Maksimum 20 təkrar',
    'queue': '**Növbəni Göstər** - Cari musiqi növbəsini göstərir.\n\n**İstifadə:** `[prefix]queue`\n**Qeyd:** Növbədəki mahnıları göstərir',
    'music': '**Musiqi Kömək** - Bütün mövcud musiqi əmrlərini göstərir.\n\n**İstifadə:** `[prefix]music`\n**Qeyd:** Musiqi əmr köməyini göstərir',
}

music_categories = {
    '🎵 MUSİQİ': ['play', 'vplay', 'playforce', 'vplayforce', 'pause', 'resume', 'skip', 'end', 'loop', 'queue', 'music'],
}
