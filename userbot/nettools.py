...
async def http_ping(client: Client, message: Message):
    ...
    msg = await edit_or_reply(message, f"🏓 **{url} ping edilir...**\n\n⏳ Bağlantı sürəti yoxlanılır...")
    try:
        async with aiohttp.ClientSession() as session:
            start = time.perf_counter()
            async with session.get(url, timeout=5) as resp:
                elapsed = (time.perf_counter() - start) * 1000
                status = "✅ Əla" if elapsed < 100 else "🟡 Yaxşı" if elapsed < 300 else "🔴 Yavaş"
                await msg.edit(f"✅ <b>Ping uğurlu oldu</b>\n\n⏱️ <b>Gecikmə:</b> <code>{elapsed:.2f} ms</code>\n🔗 <b>URL:</b> <code>{url}</code>\n📊 <b>Status:</b> {status}")
    except Exception as e:
        await msg.edit(f"❌ <b>Ping uğursuz oldu</b>\n\n⚠️ <b>Xəta:</b> <code>{str(e)}</code>\n🔗 <b>URL:</b> <code>{url}</code>\n\n💡 URL-in düzgün və əlçatan olduğunu yoxlayın.")
...
async def tcp_test(client: Client, message: Message):
    ...
    if len(args) < 3:
        await edit_or_reply(message, f"❌ **Yanlış format**\n\n💡 **İstifadə:** `{HARDCODED_PREFIXES[0]}tcp <host> <port>`\n📝 **Nümunə:** `{HARDCODED_PREFIXES[0]}tcp google.com 443`")
        return
    host, port = args[1], int(args[2])
    msg = await edit_or_reply(message, f"🔌 **TCP bağlantısı yoxlanılır...**\n\n🎯 <b>Host:</b> <code>{host}</code>\n🔌 <b>Port:</b> <code>{port}</code>\n\n⏳ Zəhmət olmasa gözləyin...")
    try:
        reader, writer = await asyncio.wait_for(asyncio.open_connection(host, port), timeout=5)
        writer.close()
        await writer.wait_closed()
        await msg.edit(f"✅ <b>TCP bağlantısı uğurlu oldu</b>\n\n🎯 <b>Host:</b> <code>{host}</code>\n🔌 <b>Port:</b> <code>{port}</code>\n✅ <b>Status:</b> Əlçatandır\n\n💡 Server onlaydır və bağlantıları qəbul edir.")
    except Exception as e:
        await msg.edit(f"❌ <b>TCP bağlantısı uğursuz oldu</b>\n\n🎯 <b>Host:</b> <code>{host}</code>\n🔌 <b>Port:</b> <code>{port}</code>\n⚠️ <b>Xəta:</b> <code>{str(e)}</code>\n\n💡 Server oflayn ola bilər və ya bağlantıları bloklayır.")
...
async def async_speedtest(client: Client, message: Message):
    msg = await edit_or_reply(message, "📡 **Sürət testi işə salınır...**\n\n⏳ Bu 30-60 saniyə çəkə bilər\n🔍 Ən yaxşı server axtarılır...")
    try:
        ...
        await msg.edit(f"📡 <b>Sürət Testi Nəticələri</b>\n\n"
                       f"🔽 <b>Yükləmə:</b> <code>{download_mbps:.2f} Mbps</code>\n"
                       f"🔼 <b>Göndərmə:</b> <code>{upload_mbps:.2f} Mbps</code>\n\n"
                       f"📊 <b>Keyfiyyət:</b> {'Əla ✅' if download_mbps > 50 else 'Yaxşı 🟡' if download_mbps > 10 else 'Orta 🔴'}")
    except Exception as e:
        await msg.edit(f"❌ <b>Sürət testi uğursuz oldu</b>\n\n⚠️ <b>Xəta:</b> <code>{str(e)}</code>\n\n💡 İnternet bağlantınızı yoxlayın və yenidən cəhd edin.")
...
**İstifadə:** `{prefix}calc <ifadə>`

**Dəstəklənən Əməliyyatlar:**
• Əsas: `+`, `-`, `*`, `/`, `%`, `**` (üstəlmə)
• Funksiyalar: `sqrt()`, `sin()`, `cos()`, `tan()`, `log()`, `abs()`
• Sabitlər: `pi`, `e`
...
• `{prefix}calc abs(-42)` (mütləq qiymət)
• `{prefix}calc pi * 2` (pi sabiti)
        ...
        for node in ast.walk(parsed):
            if not isinstance(node, _SAFE_NODES):
                return await edit_or_reply(
                    message,
                    f"❌ **Təhlükəsiz olmayan əməliyyat aşkarlandı**\n\n"
                    f"⚠️ İfadədə icazə verilməyən konstruksiya var: "
                    f"`{type(node).__name__}`\n\n"
                    f"💡 Mövcud funksiyalar üçün `{HARDCODED_PREFIXES[0]}calc` istifadə edin"
                )
            ...
            if isinstance(node, ast.Call):
                if not isinstance(node.func, ast.Name):
                    return await edit_or_reply(
                        message,
                        f"❌ **Təhlükəsiz olmayan əməliyyat aşkarlandı**\n\n"
                        f"⚠️ Yalnız birbaşa funksiya çağırışlarına icazə verilir\n\n"
                        f"💡 Mövcud funksiyalar üçün `{HARDCODED_PREFIXES[0]}calc` istifadə edin"
                    )
                if node.func.id not in safe_namespace:
                    return await edit_or_reply(
                        message,
                        f"❌ **Təhlükəsiz olmayan əməliyyat aşkarlandı**\n\n"
                        f"⚠️ `{node.func.id}` funksiyasına icazə verilmir\n\n"
                        f"💡 Mövcud funksiyalar üçün `{HARDCODED_PREFIXES[0]}calc` istifadə edin"
                    )
        ...
        await edit_or_reply(
            message,
            f"❌ **Kalkulyator xətası**\n\n"
            f"⚠️ **Xəta:** `{str(e)}`\n\n"
            f"💡 Kömək və nümunələr üçün `{HARDCODED_PREFIXES[0]}calc` istifadə edin"
        )
