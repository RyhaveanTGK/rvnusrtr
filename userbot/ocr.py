...
async def extract_text(client, message, language='eng'):
    ...
    if not (reply.photo or reply.document):
        return await message.edit("❗ Şəkilə/sənədə cavab verin")
    ...
    try:
        ...
        if reply.document and not reply.document.mime_type.startswith('image'):
            return await message.edit("❗ Sənəd şəkil formatında olmalıdır")
...
async def ocr_handler(client, message):
    try:
        ...
        # Show progress
        progress_msg = await message.reply_text(f"🔍 Mətn çıxarılır ({lang})...")
        ...
        await progress_msg.edit_text(
            f"📝 Çıxarılan mətn ({lang}):\n\n{text[:4000]}" +
            ("\n[...kəsildi]" if len(text) > 4000 else "")
        )
        ...
        await message.edit_text(
            f"⚠️ OCR Xətası\n"
            f"İstifadə: /ocr [dil]\n"
            f"Dəstəklənən dillər: eng, spa, fra, və s.\n"
            f"Xəta: {str(e)}"
        )
