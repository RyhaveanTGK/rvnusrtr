...
async def schedule_message(client: Client, message: Message):
    ...
    if len(command_parts) < 4:
        await message.reply("❌ **Yanlış format!**\nİstifadə edin: `/schedule <username/chatid> <vaxt(HH:MM:SS və ya HH:MM:SS:CC)> <mesaj>`")
        return
    ...
    try:
        ...
        await message.reply("❌ **Yanlış vaxt formatı!**\nHH:MM:SS və ya HH:MM:SS:CC formatını istifadə edin (24 saatlıq), burada CC santisaniyədir (00-99).")
        return
    ...
    try:
        ...
        if target.lstrip('-').isdigit():
            ...
            try:
                ...
                await message.reply(f"❌ **Xəta:** İstifadəçi/söhbət tapılmadı: {target}")
                return
    except Exception as e:
    ...
    try:
        ...
        await message.reply(
            f"✅ **Mesaj planlandı!**\n"
            f"📝 Kimə: `{target}`\n"
            f"🕒 Mesajınız bu vaxtda göndəriləcək: `{mess.date}`\n"
            f"⏰ Sistem saat qurşağı: `{system_timezone_name}`\n"
            f"📃 Mesaj: `{msg_content[:30]}{'...' if len(msg_content) > 30 else ''}`"
        )
        
    except Exception as e:
        await message.reply(f"❌ **Mesaj planlaması uğursuz oldu!**\n⚠️ Xəta: `{str(e)}`")
