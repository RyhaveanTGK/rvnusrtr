...
async def spam(client, message):
    if message.chat.id==-1001806816712 or is_admin(message.chat.id):
       return await message.edit_text("Sahibimin/ağamın Söhbətində/Qrupunda kimi spam etməyə çalışırsan?\n\n**Rədd ol!!!**")
    if len(message.text.split()) <3 :
       return await bot.send_message(client.me.id, f"**İSTİFADƏ NÜMUNƏSİ:** /spam 5 salam`")
    try:
        amount = int(message.command[1])
    except ValueError:
        return await bot.send_message(client.me.id, "**XƏTA:** Say rəqəm olmalıdır.")
    if amount < 1 or amount > 500:
        return await bot.send_message(client.me.id, "**XƏTA:** Say 1 ilə 500 arasında olmalıdır.")
        ...
        try:
          if message.reply_to_message:
            ...
            sent = await client.send_message(message.chat.id, text)
                ...
                mg = await bot.send_message(client.me.id, f"**XƏTA:** `{e}`")
...
async def delaySpam(client, message):
    if len(message.command) < 4:
        return await message.edit( "Ən azı spam etmək üçün bir şey ver.")

    reply_to = message.reply_to_message.id if message.reply_to_message else None
    try:
        count = int(message.command[1])
    except ValueError:
        return await message.edit("Spam üçün etibarlı say (float) ver.")

    try:
        delay = float(message.command[2])
    except ValueError:
        return await message.edit( "Spam üçün etibarlı gecikmə (int) ver.")
...
async def mentionall(client, message):
    ...
    if not direp and not args:
        return await message.reply("**Bir mesaj yaz və ya mesaja cavab ver!**")
...
async def cancel_spam(client, message):
    if not message.chat.id in spam_chats:
        return await message.edit("**Görünür, burada aktiv tagall yoxdur.**")
    else:
        try:
            spam_chats.remove(message.chat.id)
        except Exception as e:
            logger.debug(f"cancel_spam: removing chat failed: {e}")
        return await message.edit("**Qeyd ləğv edilir.**")
