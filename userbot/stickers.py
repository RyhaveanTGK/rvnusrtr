...
async def tinying(client ,message):
    ...
    if not (reply and (reply.media)):
        return await message.edit_text( "**Zəhmət olmasa hər hansı stikerə cavab verin!**")
...
async def memify(client, message):
    if not message.reply_to_message_id:
        await message.edit_text("**Hər hansı şəkil və ya stikerə cavab verin!**")
        return
    ...
    if len(text) < 1:
        return await msg.edit(f"Zəhmət olmasa `/mmf <mətn>` istifadə edin")
...
async def kang(client, message):
    ...
    RYHAVEAN = await message.edit_text("`Stiker klonlanır ahh...`")
    media_ = None
    emoji_ = None
    is_anim = False
    is_video = False
    ...
    if replied and replied.media:
        if replied.photo:
            ...
            if not replied.sticker.file_name:
                await RYHAVEAN.edit("**Stikerin adı yoxdur!**")
                return
            ...
            await RYHAVEAN.edit("**Dəstəklənməyən Fayl**")
            return
        media_ = await client.download_media(replied, file_name="downloads/")
    else:
        await RYHAVEAN.edit("**Zəhmət olmasa Şəkil/GIF/Stiker Mediasına cavab verin!**")
        return
    if media_:
        ...
        while True:
            ...
            if exist.set.count >= limit:
                ...
                await RYHAVEAN.edit(
                    f"`Stiker Paketi Doludur, {pack} Nömrəli Yeni Paket Yaradılır`"
                )
                continue
            break
        if exist is not False:
            try:
                ...
                return await RYHAVEAN.edit(f"**XƏTA:** `{e}`")
            ...
            while limit in await get_response(message, client):
                ...
                if is_video:
                    ...
                    packnick += " (Video)"
                await RYHAVEAN.edit(
                    "`Stiker Paketi Doludur, Yeni Paket Yaradılır "
                    + str(pack)
                )
                await client.send_message("stickers", packname)
                await asyncio.sleep(2)
                if await get_response(message, client) == "Invalid pack selected.":
                    ...
                    await asyncio.sleep(2)
                    await RYHAVEAN.edit(
                        f"**Stiker Uğurla Əlavə Edildi!**\n 🔥 **[BURAYA KLİKLƏYİN](https://t.me/addstickers/{packname})** 🔥\n**Stikerlərdən İstifadə Etmək Üçün**"
                    )
                    return
            await client.send_document("stickers", media_)
            await asyncio.sleep(2)
            if (
                ...
                await RYHAVEAN.edit(
                    "**Stiker Əlavə Edilə Bilmədi, Stikerinizi Əlavə Etmək Üçün @Stickers Botundan İstifadə Edin.**"
                )
                return
            await client.send_message("Stickers", emoji_)
            await asyncio.sleep(2)
            ...
            await RYHAVEAN.edit("`Yeni Stiker Paketi Yaradılır`")
            try:
                await client.send_message("Stickers", cmd)
            except YouBlockedUser:
                await client.unblock_user("stickers")
...
async def duck_command_handler(client, message):
    ...
    USERBOT = await edit_or_reply(message, f"╭── {Msg.EMOJI_NOTE} SİTAT ──╮\n┃ {Msg.EMOJI_LOADING} Sitat hazırlanır...\n╰━━━━━━━━━━━━━━━━━━━━╯")
    ...
    try:
        ...
        if hasattr(user, 'id') and is_admin(user.id):
            return await USERBOT.edit_text(
                "Sən məndən ağamın və yaradıcımın saxta sitatını yaratmağımı xahiş edirsən.\nOna görə də bunu etməyəcəyəm...**Rədd ol!!**"
            )
        ...
        try:
            await (apps.get("app") or client).send_message(client.me.id, f"Sitat işləyicisində XƏTA: {error_msg}")
