...
def build_media_caption(from_user, chat, message, is_private, include_caption=True, recipient=None):
    ...
    """
    caption = f"📥 **Media Saxlanıldı**\n\n"
    caption += f"👤 **Kimdən:** {from_user.first_name}"
    if from_user.last_name:
        caption += f" {from_user.last_name}"
    if from_user.username:
        caption += f" (@{from_user.username})"
    caption += f"\n🆔 **İstifadəçi ID:** `{from_user.id}`\n"

    if is_private:
        if recipient is not None:
            caption += f"\n👥 **Kimə:** {recipient.first_name}"
            if recipient.last_name:
                caption += f" {recipient.last_name}"
            if recipient.username:
                caption += f" (@{recipient.username})"
            caption += f"\n🆔 **Qəbuledici ID:** `{recipient.id}`\n"
        caption += f"💬 **Söhbət:** Şəxsi Söhbət\n"
    else:
        caption += f"💬 **Söhbət:** {chat.title or 'Naməlum'}\n"
        if chat.username:
            caption += f"🔗 **İstifadəçi adı:** @{chat.username}\n"

    caption += f"🆔 **Söhbət ID:** `{chat.id}`\n"
    caption += f"#️⃣ **Mesaj ID:** `{message.id}`\n"

    if message.date:
        caption += f"📅 **Tarix:** {message.date.strftime('%Y-%m-%d %H:%M:%S')}\n"
        ...
        try:
            ...
            try:
                ...
                if target_message.media:
                    ...
                    msg = await client.send_message("me", f"╭── 📥 YÜKLƏNİR ──╮\n┃ ⏳ Zəhmət olmasa gözləyin...\n╰━━━━━━━━━━━━━━━━━━━━╯")
                    type_of = "Yüklənir"
                    ...
                    details = f"📥 **Mesaj Saxlanıldı**\n\n"
                    details += f"👤 **Kimdən:** {from_user.first_name}"
                    if from_user.last_name:
                    ...
                    if is_private:
                        details += f"💬 **Söhbət:** Şəxsi Söhbət\n"
                    else:
                        details += f"💬 **Söhbət:** {chat.title or 'Naməlum'}\n"
                        if chat.username:
                            details += f"🔗 **İstifadəçi adı:** @{chat.username}\n"

                    details += f"🆔 **Söhbət ID:** `{chat.id}`\n"
                    details += f"#️⃣ **Mesaj ID:** `{target_message.id}`\n"

                    if target_message.date:
                        details += f"📅 **Tarix:** {target_message.date.strftime('%Y-%m-%d %H:%M:%S')}\n"
            ...
            try:
                await client.send_message("me", styled_error(f"Xəta: {e}"))
            except Exception:
                pass
