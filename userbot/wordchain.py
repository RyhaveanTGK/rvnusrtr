    ...
    if "Turn:" in text:
        try:
            ...
            if first_mention and first_mention.id == client.me.id:
                ...
                if not filtered_words:
                    await bot.send_message(client.me.id,f" Uyğun söz tapılmadı.")
                    return
        ...
        timeout=4
           ...
           await bot.send_message(client.me.id,f"XƏTA: {e}")
...
async def reset_used_words(client, message):
    ...
    if chat_id in used_words:
        ...
        await message.reply("Bütün istifadə olunmuş sözlər sıfırlandı.")
    else:
        await message.reply("Sıfırlanacaq söz yoxdur.")
