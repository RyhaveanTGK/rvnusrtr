...
def format_3d_results(grid_2d, results_3d, title):
    ...
    if not results_3d:
        out += "_Yazıda heç bir tapmaca ipucu tapılmadı._\n"
        return out
    ...
    for ci, layer in enumerate(results_3d):
        ...
        if not matches:
            out += "   _Uyğun namizəd tapılmadı._\n"
        else:
            ...
            if remaining > 0:
                out += f"  _…və daha {remaining} etibarlı söz_\n"
        out += "\n"
...
async def solve_wordgrid(client: Client, message: Message):
    ...
    if not target_msg or not target_msg.photo:
        await message.reply(
            "`.solvegrid` ilə şəkilə cavab ver.\n"
            "Yaxud hərfləri əl ilə yazmaq üçün `.grid` istifadə et."
        )
        return

    m = await message.reply(f"{Msg.EMOJI_DOWNLOAD} Şəkil yüklənir…")
    ...
    try:
        ...
        await m.edit(
            f"{Msg.EMOJI_SEARCH} **{len(clues)}** ipucu tapıldı. "
            f"Şəbəkə çıxarılır (AI Vision → Tesseract ehtiyat)…"
        )

        raw_rows = extract_grid_from_image(photo_path)
        if not raw_rows:
            await m.edit(
                "❌ OCR şəbəkəni çıxara bilmədi.\n"
                "Hərfləri əl ilə daxil etmək üçün `.grid` istifadə et."
            )
            return
        ...
        if not grid_2d:
            await m.edit(
                "❌ Şəbəkə sətirləri normallaşdırıla bilmədi.\n"
                "Hərfləri əl ilə daxil etmək üçün `.grid` istifadə et."
            )
            return
        ...
        await m.edit(
            f"{Msg.EMOJI_SUCCESS} **{nrows}×{ncols}** şəbəkə çıxarıldı. Həll edilir…"
        )
        ...
        if "HARD MODE" in cap_up:
            title = f"{Msg.EMOJI_FIRE} Çətin Rejim — 3D Şəbəkə Həlledicisi"
        elif "WORD GRID" in cap_up:
            title = f"{Msg.EMOJI_FIRE} Söz Şəbəkəsi — 3D Şəbəkə Həlledicisi"
        else:
            title = f"{Msg.EMOJI_PUZZLE} Söz Şəbəkəsi — 3D Şəbəkə Həlledicisi"
        ...
        await m.edit(f"{Msg.EMOJI_ERROR} Xəta: {ai_backend.scrub(str(e))}")
    finally:
...
async def manual_grid(client: Client, message: Message):
    ...
    if not grid_text:
        await message.reply(
            "**İstifadə:** tapmaca mesajına cavab ver, sonra:\n"
            "```\n.grid\nSHOWSUBC\nCORFEDDO\nSAWXELEM\n"
            "BANNDOSM\nTBCEEVUO\nFHKRERLN\n"
            "MSOREDSS\nAYYVVDLQ```"
        )
    ...
    m = await message.reply(f"{Msg.EMOJI_GEAR} Əl ilə daxil edilən şəbəkə emal edilir…")
    ...
    try:
        ...
        if not raw_rows:
            await m.edit(f"{Msg.EMOJI_ERROR} Etibarlı sətir tapılmadı.")
            return

        grid_2d = build_2d_grid(raw_rows)
        if not grid_2d:
            await m.edit(f"{Msg.EMOJI_ERROR} Daxil edilən məlumatdan şəbəkə qurula bilmədi.")
            return
        ...
        nrows, ncols = len(grid_2d), len(grid_2d[0])
        await m.edit(
            f"{Msg.EMOJI_SUCCESS} **{nrows}×{ncols}** şəbəkə yükləndi. "
            f"**{len(clues)}** ipucu ilə axtarılır…"
        )

        valid_words = load_valid_words()
        results_3d = search_grid_3d(grid_2d, clues, valid_words)
        title = f"{Msg.EMOJI_PUZZLE} Söz Şəbəkəsi — 3D Şəbəkə Həlledicisi (Əl ilə)"
        response = format_3d_results(grid_2d, results_3d, title)
        await m.edit(response)

    except Exception as e:
        logger.error(f"Error processing manual grid: {e}")
        await m.edit(f"{Msg.EMOJI_ERROR} Xəta: {e}")
