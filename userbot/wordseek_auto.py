...
async def show_game_info(client: Client, message: Message):
    ...
    if chat_id not in ACTIVE_GAMES:
        await message.reply("Aktiv avtomatik oyun yoxdur.")
        return
...
📊 **Oyun Məlumatı**
━━━━━━━━━━━━━━━━━━━
• **Vəziyyət:** {'Aktiv' if game['game_active'] else 'Bitib'}
• **Edilən Təxminlər:** {game['guesses_made']}/30
• **Qalan Cəhdlər:** {game['attempts_left']}
• **Təsdiqlənmiş Hərflər:** {len(game['position_hints'])}
• **İstisna Edilmiş Hərflər:** {len(game['excluded_letters'])}
• **Məlum Hərflər:** {len(game['known_letters'])}

📍 **Mövqe İpuçları:**
{', '.join(f'{k}: {v}' for k, v in sorted(game['position_hints'].items())) if game['position_hints'] else 'Yoxdur'}

🚫 **İstisna edilənlər:** {', '.join(sorted(game['excluded_letters'])) if game['excluded_letters'] else 'Yoxdur'}
    """
...
async def wordseek_info(client: Client, message: Message):
    ...
    info = (
        "🎮 **WordSeek Avtomatik Oyun**\n"
        "━━━━━━━━━━━━━━━━━━━\n"
        "• **Necə başlamalı:** Aşağıdakı işə salan sözlərdən birini göndər\n"
        f"• **İşə salan sözlər:** {words}\n"
        f"• **Dəstəklənən söz uzunluqları:** {supported}\n"
        "• **Qeyd:** Bot ilk sözdən sonra avtomatik təxmin edəcək\n"
    )
    await message.reply(info)
